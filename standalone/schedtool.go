...
func SwitchFunc(pid int, core string){
	...
	//	fmt.Println("logs switch func cmd.Start: %v", err)
	//}
	err := cmd.Start()
	if err != nil{
		log.Fatal(err)
...
func UpdateFunc(pid int, core string, p string){
	...
	//cmd = exec.Command("schedtool","N", "-a", core, strconv.Itoa(pid))
	//if err := cmd.Start(); err != nil{
	//		log.Fatal("logs update func cmd.Start: %v", err)
	//}
	err := cmd.Start()
        if err != nil{
                log.Fatal(err)
	...
	for{
		...
		case x,_ := <-in:
			...
			if x.Pid == -1{
				...
				for job, pid := range(m){
					//actions.Lock()
					if jobs[actions.m[job].Id] == 1{
							update_core := "0x"+string(strconv.FormatInt(cfs_value, 16))
							SwitchFunc(pid, update_core)
					}
					//actions.Unlock()
				}
			}else{
				fmt.Println("logs cfs in", x)
				m[x.Job] = x.Pid
			}
			**/
			if a >= 1000000 && x.Pid == -1{
				fmt.Println("logs large number")
				a = 0
			}
		}
	}
